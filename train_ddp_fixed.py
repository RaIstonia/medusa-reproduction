...
def main(args):
    ...
    # 注意：modeling_medusa.py 中需要确保 Base Model 输出转为 float32
    model.train()
    ...
    dataloader = train_dataset.set_attrs(
        ...
        shuffle=True,
        num_workers=args.num_proc // 2 # 动态设置 worker
    )

    # --- Step 4: 训练配置 ---
    optimizer = optim.AdamW(model.medusa_head.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    medusa_decay = 0.8
